...
function Home() {
  ...
  const onOverlay = () => navigate("/");
  const clickedMovie =
    movieMatch?.params.id &&
    data?.results.find((movie) => movie.id + "" === movieMatch.params.id);
  const [pageIndex, setPageIndex] = useRecoilState(indexState);
  const randomIndex = () =>
    setPageIndex(Math.floor(Math.random() * (totalMovie as any)));
  return (
    <Wrapper>
      {isLoading ? (
        ...
        <>
          ...
          >
            <Title>{data?.results[pageIndex].title}</Title>
